    ...
    Node * constructBST(int preorder[], int &start, int min,int max,int n)
    {
       ...
       start++;
       root->left=constructBST(preorder,start,min,val-1,n);
       root->right=constructBST(preorder,start,val+1,max,n);
    ...
    Node *root=constructBST(preorder, start, INT_MIN,INT_MAX,n);
    inorder(root);
